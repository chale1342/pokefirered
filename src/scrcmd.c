...
// Like callnative, but the function is passed the script context
...
// Put the follower Pokemon (if present and visible) into its Poke Ball.
// Returns the follower object, or NULL if there was nothing to hide.
static struct ObjectEvent * ScriptHideFollower(void)
{
...
// Buffers the nickname of the first non-fainted party mon (i.e. the follower)
...
// Plays the cry of the first non-fainted party mon (i.e. the follower)
...
// Script command (see the `hidefollower` macro): hides the follower Pokemon
// in its Poke Ball; if `wait` is nonzero, waits for the animation to finish.
