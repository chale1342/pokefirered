...
#include "global.h"
#include "data.h" // struct FollowerMsgInfo / FollowerMessagePool
...
// Kanto-flavored condition set; emerald-only entries
// (contests, Battle Frontier, Hoenn maps/music) were dropped or remapped.
enum {
    COND_MSG_CELEBI,
    COND_MSG_FIRE,
    COND_MSG_INDIGO_PLATEAU,
    COND_MSG_MT_EMBER,
    COND_MSG_DAY_CARE,
...
// The declarations below belong to the core follower engine
// (src/event_object_movement.c, ported separately). They live here because
// include/event_object_movement.h and include/constants/event_objects.h are
// owned by that port. Signatures copied from merrp's event_object_movement.h.
#ifndef OBJ_EVENT_ID_FOLLOWER
...
// Note: static in vanilla pokefirered; the core follower port makes it public
